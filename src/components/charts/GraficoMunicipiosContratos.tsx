...
import type { MunicipioContrato } from '../../types';

interface Props {
  data?: MunicipioContrato[];
}

const COLORS = ['#34a853', '#2e9549', '#25883f', '#1e7a36', '#166c2d'];

const GraficoMunicipiosContratos: React.FC<Props> = ({ data = [] }) => (
  <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-none flex flex-col gap-3.5 hover:shadow-md transition-shadow">
    <div className="flex items-baseline justify-between gap-2">
      <h3 className="text-[14px] font-semibold text-gray-900">Municípios que Mais Contratam</h3>
    ...
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
        ...
        />
        <Bar dataKey="contratos" radius={[4, 4, 0, 0]} maxBarSize={48}>
          {data.map((_, i) => (
...
export default GraficoMunicipiosContratos;
